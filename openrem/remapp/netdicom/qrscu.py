            ...
            for existing_study in existing_studies:
                ...
                for series_rsp in study.dicomqrrspseries_set.all():
                    if series_rsp.modality == "SR":
                        ...
                        _query_images(ae, remote, assoc, series_rsp, query, True) # Only check first image of series
                        _remove_image_sop_uids(
                            series_rsp,
                            query_id_8,
                            study_number,
    ...
    for study in study_rsp:
        ...
        if all_mods["MG"]["inc"] and "MG" in study.get_modalities_in_study():
            ...
            if series.count() == 0:
                ...
                continue
            logger.debug(
                f"{query_id_8} Found {loaded_sop_classes}. Keeping them all."
            )
            for serie in series:
        ...
        if status:
            if status.Status == 0x0000:
                ...
                if initial_image_only and instance_number is None: 
                    ...
                    if image_count == 0:
                        _query_images(ae, remote, assoc, seriesrsp, query, True, msg_id, 0)
