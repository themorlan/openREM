        ...
        try:
            ...
            if flower.status_code == 200:
                ...
                if u"active" in stage:
                    return render_to_response('remapp/celery_tasks.html', {'tasks': active_tasks, 'celeryTable': 'Active'},
                                              context_instance=RequestContext(request))
                elif u"recent" in stage:
                    return render_to_response('remapp/celery_tasks_complete.html', {'tasks': recent_tasks, 'celeryTable': 'Recent'},
                                              context_instance=RequestContext(request))
                elif u"older" in stage:
                    return render_to_response('remapp/celery_tasks_complete.html', {'tasks': older_tasks, 'celeryTable': 'Older'},
                                              context_instance=RequestContext(request))
        except requests.ConnectionError:
            admin = _create_admin_dict(request)
