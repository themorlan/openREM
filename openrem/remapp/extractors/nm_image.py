...
def _check_replace(study, attr, value, study_id, is_not_equal):
    ...
    if not current:
        ...
        logger.warn(f"Found different values for {attr} on image and existing object for study {study_id}")
    return False
        
def _try_set_value(where, kw_database, dataset, kw_dataset, study_id, 
    get_with=get_value_kw, mod=lambda x: x, is_not_equal = lambda x, y : x != y):
...
def _isotope(study, dataset):
    ...
    radio = study.radiopharmaceuticalradiationdose_set.get().radiopharmaceuticaladministrationeventdata_set.get()
    _try_set_value(radio, "radionuclide", dataset, "RadionuclideCodeSequence", study_id, _code_getter, _code_handler)
    _try_set_value(radio, "radionuclide_half_life", dataset, "RadionuclideHalfLife", study_id, 
    ...
    _try_set_value(radio, "radiopharmaceutical_stop_datetime", dataset, 
        "RadiopharmaceuticalStopDateTime", study_id, get_date_time)

    def build_datetime(x):
        return datetime.combine(study.study_date, x.time())
    _try_set_value(radio, "radiopharmaceutical_start_datetime", dataset, "RadiopharmaceuticalStartTime",
...
def nm_image(file: str):
    ...
    if dataset.SOPClassUID in ["1.2.840.10008.5.1.4.1.1.128", "1.2.840.10008.5.1.4.1.1.20"]:
        _nm2db(dataset)
    else:
