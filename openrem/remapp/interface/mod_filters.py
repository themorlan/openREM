...
class RFSummaryListFilter(django_filters.FilterSet):
    ...
    acquisition_protocol = \
        django_filters.CharFilter(lookup_type='icontains', label=u'Acquisition protocol',
    ...
    test_data = django_filters.ChoiceFilter(lookup_type='isnull', label=u"Include possible test data",
                                            name='patientmoduleattr__not_patient_indicator',
                                            choices=TEST_CHOICES, widget=forms.Select)
...
class CTSummaryListFilter(django_filters.FilterSet):
    ...
    accession_number = django_filters.MethodFilter(action=custom_acc_filter, label=u'Accession number')
    study_dlp_min = \
        django_filters.NumberFilter(lookup_type='gte', label=u'Min study DLP',
                                    name='ctradiationdose__ctaccumulateddosedata__ct_dose_length_product_total')
    study_dlp_max = \
        django_filters.NumberFilter(lookup_type='lte', label=u'Max study DLP',
                                    name='ctradiationdose__ctaccumulateddosedata__ct_dose_length_product_total')
    display_name = django_filters.CharFilter(lookup_type='icontains', label=u'Display name',
                                             name='generalequipmentmoduleattr__unique_equipment_name__display_name')
    test_data = django_filters.ChoiceFilter(lookup_type='isnull', label=u"Include possible test data",
    ...
    ct_acquisition_type = \
        django_filters.MultipleChoiceFilter(
            lookup_type='iexact', label=u'Acquisition type restriction',
...
def ct_acq_filter(filters, pid=False):
    ...
    if 'acquisition_protocol' in filters and (
        ...
        if 'ct_acquisition_type' in filters:
            try:
                ...
                pass
        filtered_include = events.values_list(
    ...
    if filtered_include:
        studies = studies.filter(study_instance_uid__in=filtered_include)
    if pid:
...
class MGSummaryListFilter(django_filters.FilterSet):
    ...
    class Meta:
        ...
        order_by = (
            ...
            ('-projectionxrayradiationdose__accumxraydose__accummammographyxraydose__accumulated_average_glandular_dose',
             'Accumulated AGD'),
            )
