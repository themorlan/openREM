...
class ExportMammoCSVNHSBSP(TestCase):
    ...
    def test_nhsbsp(self):
        ...
        self.assertEqual(
            Counter(csvdf.Filter.tolist()),
            Counter(["Al", "Al", "Al", "Al", "Ag", "Rh", "Rh"]),
        )

        # Test numbered view codes
        self.assertEqual(
            Counter(csvdf["View code"].tolist()),
            Counter(["RCC", "RCC2", "RCC3", "RCC4", "RCC5", "ROB", "LCC"]),
        )

        # Test each row
        rcc1 = csvdf[
            csvdf["View code"].str.contains("RCC")
            & (csvdf["Filter"] == "Al")
            & (csvdf["Thickness"] == 19.0)
        ]
