...
class ImportToshibaWithDoseCheck(TestCase):
    ...
    def test_dose_check_import(self):
        ...
        series1 = study.ctradiationdose_set.get().ctirradiationeventdata_set.order_by(
            "id"
        )[0]
        series2 = study.ctradiationdose_set.get().ctirradiationeventdata_set.order_by(
            "id"
        )[1]
        ...
        self.assertAlmostEqual(
            series1_dose_check.accumulated_dlp_forward_estimate, Decimal(251.2)
        )
        ...
        self.assertAlmostEqual(
            series2_dose_check.accumulated_dlp_forward_estimate, Decimal(502.4)
        )
        self.assertAlmostEqual(
            series2_dose_check.accumulated_ctdivol_forward_estimate, Decimal(10.60)
        )
