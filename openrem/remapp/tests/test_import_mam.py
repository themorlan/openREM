...
class ImportMGImg(TestCase):
    def test_import_mg_img_ge(self):
        ...
        self.assertEqual(
            "Reciprocating grid"
            in set(
                grid.xray_grid.code_meaning
                for grid in study.projectionxrayradiationdose_set.get()
                .irradeventxraydata_set.get()
                .irradeventxraysourcedata_set.get()
                .xraygrid_set.all()
            ),
        ...
        self.assertEqual(
            ...
            .compression_force,
            Decimal(50),
        )  # not in std, recorded as presented
        self.assertEqual(
            study.projectionxrayradiationdose_set.get()
            .irradeventxraydata_set.get()
            .irradeventxraymechanicaldata_set.get()
    ...
    def test_import_mg_img_ge_pid_hashed(self):
        ...
        self.assertEqual(
            study.patientmoduleattr_set.get().patient_name,
            "fe66aceb4eb0ccbd76306a485e162cc3cad8c9312b25b002ad784f72575ae500",
        )
        self.assertEqual(
            study.patientmoduleattr_set.get().patient_id,
            "1635c8525afbae58c37bede3c9440844e9143727cc7c160bed665ec378d8a262",
        )
        self.assertEqual(
            study.accession_number,
            "8f541d3a1bdab5e197e3acb3b51419b162809c926ee7f45044aca9aef9d6e22d",
        )
...
class ImportDuplicatesMG(TestCase):
    ...
    * Rejection of third image, this time because SOPInstanceUID is recognised
    ...
    def test_duplicate_event(self):
        ...
        mg_im1_for_proc = os.path.join(
            "test_files", "MG-Im-GE_Seno_1_ForProcessing.dcm"
        )
        mg_im1_for_pres = os.path.join(
            "test_files", "MG-Im-GE_Seno_1_ForPresentation.dcm"
        )
        mg_im2_for_pres = os.path.join(
            "test_files", "MG-Im-GE_Seno_2_ForPresentation.dcm"
        )
        root_tests = os.path.dirname(os.path.abspath(__file__))
        ...
        with LogCapture(level=logging.DEBUG) as log1:
            ...
            # Check summary fields
            self.assertAlmostEqual(
                study.total_agd_left, Decimal(0.00547) * Decimal(100)
            )
            self.assertEqual(study.number_of_events, 1)
        ...
        # Check summary fields
        self.assertAlmostEqual(
            study.total_agd_left, Decimal(0.00547 + 0.01409) * Decimal(100)
        )
        ...
        with LogCapture(level=logging.DEBUG) as log2:
            ...
            self.assertEqual(GeneralStudyModuleAttr.objects.all().count(), 1)
            number_events = (
                GeneralStudyModuleAttr.objects.order_by("pk")[0]
