...
urlpatterns = [
    ...
    path(
        "{0}openrem/".format(VIRTUAL_DIRECTORY), include("remapp.urls")
    ),  # is this actually necessary?
    path("{0}admin/".format(VIRTUAL_DIRECTORY), admin.site.urls),
    # Login / logout.
    path(
        "{0}login/".format(VIRTUAL_DIRECTORY),
        auth.views.LoginView.as_view(),
        name="login",
    ),
    path(
        "{0}logout/".format(VIRTUAL_DIRECTORY), remapp.views.logout_page, name="logout"
    ),
]
